...
const Comment = ({ data, setModal }) => {
  ...
  return (
    <div className="  scale-up-ver-bottom bottom-0 md:w-[95%] p-0 m-auto md:h-[30%]  bg-indigo-300">
      {user ? (
        <div>
          <label className="block pt-3 fontFamily tracking-wide text-cyan-50 text-medium font-bold m-2">
            Write your comment here
          </label>
          <XCircleIcon
            onClick={() => setModal(false)}
            className="absolute right-1 cursor-pointer text-black top-[3px] w-6 h-6 md:w-9 md:h-9"
          />
          <textarea
